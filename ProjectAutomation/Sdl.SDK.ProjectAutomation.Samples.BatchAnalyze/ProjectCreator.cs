namespace Sdl.SDK.ProjectAutomation.Samples.BatchAnaylze
{
	...
	public class ProjectCreator
	{
...

		{
			...
			FileBasedProject newProject = new FileBasedProject(GetProjectInfo(srcLocale, trgLocale));

			#endregion

			#region "CallAddFiles"

			AddFiles(newProject, docFolder, recursion);
			...
			if (publishToServer)
			{
				...
				AddServerTm(newProject, "http://MyTMServerAddress", "/MyOrganizationPath", tmFile, false, "MyUserName", "MyPassword");
				#endregion
			}
			else
			{
				#region "CallAddTm"
				AddTm(newProject, tmFile);
				#endregion
			}


			#region "CallConvert"
			ConvertFiles(newProject);
			#endregion
			...
			RunFileAnalysis(newProject, trgLocale);
			#endregion

			#region "GenerateReports"
			CreateReports(newProject, docFolder);
			#endregion
		...
		private ProjectInfo GetProjectInfo(string srcLocale, string trgLocale)
		{
			...
			info.LocalProjectFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments),
				studioVersionService.GetStudioVersion().StudioDocumentsFolderName,
				"Projects",
				info.Name);

			info.SourceLanguage = new Language(CultureInfo.GetCultureInfo(srcLocale));
			info.TargetLanguages = new Language[] { new Language(CultureInfo.GetCultureInfo(trgLocale)) };
		...
		private void RunFileAnalysis(FileBasedProject project, string trgLocale)
		{
			...
			reportId = analyzeTask.Reports[0].Id;
		}
